...
json gen_response(const std::string& text, const string& tts, const json& buttons, const json* current_session = nullptr, const bool end_session = false)
{
	json response = {
		{"response", {
			{"buttons", buttons},
...
void AliceSkill(const Request& req, Response& res)
{
	...
	else
	{
		...
		{
			cout << "cart: " << (*cur_session)["cart"] << endl;
			...
			|| command.find("убрать из корзины") == 0 || command.find("убери из корзины") == 0)
		{
			...
			{
				cout << req_json["request"]["nlu"]["tokens"][i].get<string>() << endl;
